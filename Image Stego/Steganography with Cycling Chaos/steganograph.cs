...
namespace Steganography_with_Cycling_Chaos
{
    class steganograph
    {
        ...
        private int[] Convert_TO_Binary2(int a)
        {
        ...
        {
            int[] output = new int[entry_data.Length * 8+32];
            int index = 0;
            for (int ii = 0; ii < 2; ii++)
            {
                int jj = 0;
                int[] binary = Convert_TO_Binary2(entry_data[ii]);
                while (jj < binary.Length)
                {
                    output[index++] = binary[jj++];
                }
            }
            for (int ii = 2; ii < entry_data.Length; ii++)
            {
                int jj=0;
        ...
        public int[,] stegno(int[,]image,int[] inputs,ref int count,int demoniator)
        {
            restrict();
            int index = 0;
            ...
            for (int kk = 0; kk < rand_sq1.Length; kk++)
            {
                ...
                for(int ii = 0; ii < rand_sq.Length&&count<inputs.Length; ii++)
                {
                    ...
                    if ((image[rand_sq1[kk], index2] % (2 * demoniator)) < demoniator)
                        image[rand_sq1[kk], index2] += inputs[count] * demoniator;
                    else if (inputs[count] == 0)
                        image[rand_sq1[kk], index2] -= demoniator;
                    count++;
                }
        ...
        public void save_pixels(int[] p, int start,string filename,int endfile)
        {
            StreamWriter sw = new StreamWriter(filename);
            for (int i = start; i < endfile; i++)
            {
                sw.Write(p[i] + " ");
            }
            sw.Close();
